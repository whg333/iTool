...
import com.why.tool.proto.ProtocolBuffer.ByteList;

public class BytesList implements List<byte[]>, ProtobufSerializable<ByteList> {
	...
	@Override
	public void copyFrom(ByteList list) {
		int size = list.getElementsCount();
		for (int i = 0; i < size; i++) {
			this.elements.add(list.getElements(i).toByteArray());
		}
	}

	@Override
	public ByteList copyTo() {
		ByteList.Builder builder = ByteList.newBuilder();
	...
	public void parseFrom(byte[] bytes) {
		try {
			ByteList list = ByteList.parseFrom(bytes);
			copyFrom(list);
